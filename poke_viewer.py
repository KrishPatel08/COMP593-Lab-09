...
def handle_get_info_btn_click():
    ...
    poke_types = []
    for poke in poke_info['types']:
        poke_types.append(poke['type']['name']) 
    poke_type_var = ', '.join(poke_types).title()

    lbl_type_value['text'] = f"{poke_type_var}"
...
lbl_height.grid(row=0, column=0, sticky=E, pady=5)
...
lbl_type.grid(row=2, column=0, sticky=E, pady=5)
...
prg_hp.grid(row=0, column=1,pady=5, padx=(0,5))
